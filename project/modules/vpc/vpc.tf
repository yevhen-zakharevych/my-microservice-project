...
# Виділяємо публічні IP-адреси для NAT Gateway у кожній публічній підмережі
resource "aws_eip" "nat" {
  count = length(var.public_subnets) # Один EIP на кожну публічну підмережу
...
# Створюємо NAT Gateway для приватних підмереж через публічні підмережі
